...
# These are specific to the Rasberry Pi
import RPi.GPIO as GPIO

from LED_rPi import LED
from BitPlay_rPi import BitPlay
# End Raspberry Pi specific imports
...
'''
	Initialization
'''
dcsVersion = "0.4.1 (Unstable, 04-Apr-2014)"
clearScreen()
...
'''
class DCS_Pod (DCS_Device, BitPlay):
	myLeds = []
	myLedsProcess = []
	...
	def __init__ (self, adnauth, mname, mnet, mserv, mcheck, mpassword=None, mkey=None):
		...
		# Green LED
		green = LED("green", 25, False, "Green", "Green LED", 0, None, None)
		self.myLeds.append(green)

		# Red LED
		red = LED("red", 17, False, "Red", "Red LED", 0, None, None)
		self.myLeds.append(red)

		# Yellow LED
		yellow = LED("yellow", 4, False, "Yellow", "Yellow LED", 0, None, None)
	...
	***	Begin LED Management routines
	...
	'''
	*** Camera processing routines
	'''

	'''
		Shoot a video
	'''
	def shootVideo (self, video):
		status = True

		videoType = video["type"]
		videoLength = video["length"]
		videoUnits = video["units"]
		videoPan = video["pan"]

		print ("   Shooting " + videoType + " for " + str(videoLength) + " " + videoUnits + "..")

		return status

	'''
		Take some picture(s). If more than one picture is to be taken, they
			can be taken at a set interval.
	'''
	def takePictures (self, pictures):
		status = True

		nrPictures = pictures["number"]

		if (nrPictures > 1):
			interval = pictures["interval"]
		else:
			interval = 0

		print ("   Taking " + str(nrPictures) + " picture(s)..")

		return status
	'''
	*** End of camera processing routines
	'''
	...
	def processRequest (self, aNote, msgid):
		...
		if (deviceIndex == None) and (device == None):
			...
			if (req == "led"):
				...
				# We can take pictures and video in the same request
				if ("picture" in reqParams):
					picture = reqData["picture"]

				if ("video" in reqParams):
					video = reqData["video"]

				# Do things in the order of the parameters
				for action in reqParams:
					if (action == "picture"):
						takePictures(picture)
					elif (action == "video"):
						shootVideo(video)
					else:
						self.displayError("processRequest", 9999, "Invalid camera command: " + action)
			else:
...
ds9Name = "ds9"
ds9Network = "hybotics.org"
ds9CheckType = "crc64"
ds9Resources = [ "fixed", "leds", "camera" ]
ds9Password = "fbs6dE!P8u443V^@jsN*PQZ^h"
dspPublicKey = None

print ("")
print ("Creating device " + ds9Name + "." + ds9Network + ", a fixed device..")
print ("")

ds9 = DCS_Pod(adn, ds9Name, ds9Network, ds9Resources, ds9CheckType, ds9Password, ds9PublicKey)

status = ds9.connect(patterChannel)
