...
from Hybotics_Utils import getTimeZone, clearScreen
...
class DCS_Controller(DCS_Device):
	...
	'''
		Check to see if we have an authenticated device, and send a
			message to them.
	'''
	def sendAuthenticatedMessage (self, aNote, toFQN):
		...
		if (nrDevices > 0):
			...
			while (foundDevice == None) and (dcsIndex < nrDevices):
				validated = False
				dcsDevice = self.myDevices[dcsIndex]

				print ("(sendAuthenticatedMessage) device:")
				self.pp.pprint(device)

				if dcsDevice["authenticated"] and (self.deviceFQN == toFQN):
					validated = deviceAuthInfo["valid"]
					foundDevice = dcsDevice
		...
		if (validated):
			...
			status, msgid = sendMessage(toFQN, [ aType ], aNote, self.deviceAuthorization)

			if (not status) and (msgid == None):
				self.displayError("sendAuthenticatedMessage", 9999, "I could not send an authenticated message to " + toFQN)
	...
	'''
	def sendShutdown (self, deviceFQN):
		status = True
		msgid = None
		...
		if (deviceIndex == None) and (device == None):
			self.displayError("sendShutdown", 9999, deviceFQN + "is not known to me")
		else:
			...
			if self.deviceGotAuthenticated and self.shuttingDown and ("controller" in self.myResources) and (self.deviceNetwork == self.myNetwork):
				...
				status, msgid = self.sendMessage(self.deviceFQN, [ "command"] )
...
class DCS_Master(DCS_Controller):
	...
	def run (self):
		...
		while (self.running):
			...
			msg = self.getMessage(self.myFQN)
...
#		self.debuggingSendMessage = True
	...
	def writeTestMessages (self):
		...
		status, msgid = self.sendMessage(toFQN, [ "request" ])
...
hdcName = "hdc"
hdcNetwork = "hybotics.org"
hdcChecktype = "crc64"
hdcPassword = "wV0qX8T0S1M45Ew93@qcPtY-f"
hdcPubKey = None
hdcResources = ["fixed", "controller", "wifi" ]

hdc = DCS_Master(adn, hdcName, hdcNetwork, hdcResources, hdcChecktype, hdcPassword, hdcPubKey)

status = hdc.connect(patterChannel)
